...
if (!function_exists('rb_rrmdir')) {
  function rb_rrmdir($dir) {
    ...
    foreach ($items as $item) {
      ...
      if (is_dir($path)) rb_rrmdir($path);
      else @unlink($path);
    }
    @rmdir($dir);
  }
}
if (!function_exists('rb_restore_head_from_backup')) {
...
if (is_dir($iconsDirFs)) {
  ...
  if (!is_dir($iconsDirFs)) usSuccess('ReBrand: Removed icons directory.');
  else { usError('ReBrand: Failed to remove icons directory.'); $hadError = true; }
}
if (is_dir($pluginStorageDir)) {
  rb_rrmdir($pluginStorageDir);
  if (!is_dir($pluginStorageDir)) usSuccess('ReBrand: Removed plugin storage directory.');
  else { usError('ReBrand: Failed to remove plugin storage directory.'); $hadError = true; }
}

// 4) Optionally remove head_tags.php if no restore requested and file looks auto-generated
if (!$restoreHead && is_file($headFile)) {
  ...
  if (is_string($body) && strpos($body, 'Auto-generated by ReBrand plugin') !== false) {
    if (@unlink($headFile)) usSuccess('ReBrand: Removed auto-generated head_tags.php.');
    else { usError('ReBrand: Could not remove head_tags.php.'); $hadError = true; }
  }
}

// 5) Final messaging
if ($hadError) usError('ReBrand purge completed with some errors. Review messages above.');
else usSuccess('ReBrand purge completed successfully.');
