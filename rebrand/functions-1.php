...
function renderBrandingHeaderALT($domain = null)
{
  $data = getRebrandData($domain);
  if (!$data) return;
...
function renderBrandingHeader($domain = null)
{
  ...
  // Inject only social icons under header if enabled
  if ($data['icon_placement'] === 'header' || $data['icon_placement'] === 'both' || $data['icon_placement'] === 'under') {
    echo '<div class="us_brand full_screen text-center">';
    renderBrandingIcons($domain, 'header-under-logo');
    echo '</div>';
  }
